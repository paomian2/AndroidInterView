...
/**
 * @author Linxz
 * 创建日期：2020年04月10日 15:25
 * version：v4.5.4
 * 描述：多套values
 *       只能适配主流机型
 *       如果有的设备在找不到对应的vulues文件夹，则读取vulues默认的文件，导致出现不适配的情况
 *
 * {@link DimenGenerator}方式叫Smallwidth,也是生成多套文件夹，是已sw方式存在，内部使用的是dp。
 * 这套方案如果设备找不到对应的vlues文件夹，怎么读取最优的那个进行适配
 */
